#!/usr/bin/env python3
"""
FFB Frequency Analyzer  v1.2 (Optimized)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
New in v1.2:
  • 60 Hz UI refresh rate
  • FFT output truncated to display range (reduced CPU/GPU load)
...
UPDATE_INTERVAL  = 6         # new FFT every N samples  (~60 Hz at 360 Hz source)
...
class RealtimeDSP:
    ...
    def process(self, chunk, bands):
        ...
        # Compressor (runs before output gain)
        ...
        # Master / output gain — always last in chain
...
class FFTProcessor(QObject):
    ...
    def __init__(self):
        ...
        # Full Nyquist range (0–180 Hz at 360 Hz sample rate)
...
class MainWindow(QMainWindow):
    ...
    def _on_spectrum(self, freqs: np.ndarray, mag_db: np.ndarray, fx_db: np.ndarray, rms: float):
        ...
        if self._overlay_win.isVisible():
            # Compute post-compressor spectrum: fx_db shifted by live gain reduction
            gr_db = 0.0
            if hasattr(self, '_proc') and hasattr(self._proc, 'dsp'):
                env = self._proc.dsp._env
                thr = self._proc.dsp.comp_threshold_db
                ratio = self._proc.dsp.comp_ratio
                makeup = self._proc.dsp.comp_makeup_db
                if env > 1e-9 and ratio > 1.0:
                    env_db = 20.0 * math.log10(env + 1e-9)
                    if env_db > thr:
                        over = env_db - thr
                        gr_db = over * (1.0 - 1.0 / ratio)
            comp_db = fx_db - gr_db + self._proc.dsp.comp_makeup_db if hasattr(self, '_proc') else fx_db
            self._overlay_win.update_plots(freqs, mag_db, fx_db, comp_db, self._wf_buf)
    ...
    def _sync_dsp(self):
        if hasattr(self, '_proc') and hasattr(self._proc, 'dsp'):
            self._proc.dsp.master_gain_db   = 0.0  # gain now lives in comp_makeup_db
            ...
            # Update signal chain widget
            eq_active = sum(1 for b in self._eq_bands if b.enabled and abs(b.gain_db) > 0.01)
            gr_db = 0.0
            if hasattr(self._proc, 'dsp'):
                env = self._proc.dsp._env
                thr = self._comp_thr.value()
                ratio = self._comp_ratio.value()
                if env > 1e-9 and ratio > 1.0:
                    env_db = 20.0 * math.log10(env + 1e-9)
                    if env_db > thr:
                        over = env_db - thr
                        gr_db = over * (1.0 - 1.0 / ratio)
            self._signal_chain.update_state(
                self._master_gain.value(),
                self._comp_ratio.value(),
                self._comp_thr.value(),
                eq_active,
                gr_db,
            )
