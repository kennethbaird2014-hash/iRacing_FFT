...
UPDATE_INTERVAL  = 2        # new FFT every N samples
            ...
            try:
                ...
                if tick is not None and tick != last_tick:
                    ...
                    if torque is not None:
                        self.sample_ready.emit(float(torque))
                time.sleep(0.001)
...
class MainWindow(QMainWindow):
    ...
    def _detect_peaks(self):
        if self._last_freqs is None or self._last_mag_db is None:
            return

        freqs  = self._last_freqs
        mag_db = self._last_mag_db
